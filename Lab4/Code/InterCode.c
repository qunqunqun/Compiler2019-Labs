...
// translate函数
InterCodes translateTree(GramTree * root){
...
InterCodes translate_ExtDefList(GramTree* root){
    ...
    if(root->nChild == 0 || root->nChild == 1){
        ...
        InterCodes a = translate_ExtDef(root->child[0]);
        InterCodes b = translate_ExtDefList(root->child[1]);
        return link2Codes(a,b);
        // return link2Codes(
...
InterCodes translate_ExtDef(GramTree* root){
    ...
    if(root->nChild == 3){
        if(isEqual(root->child[1]->tag, "FunDec") == 1 && isEqual(root->child[2]->tag, "CompSt") == 1){
            InterCodes a = translate_FunDec(root->child[1]);
            InterCodes b = translate_CompSt(root->child[2]);
            return link2Codes(a,b);
            // return link2Codes(
...
InterCodes translate_VarList(GramTree* root){
    ...
    if(root->nChild == 1){
        ...
        InterCodes varListCode = translate_VarList(root->child[2]);
        return link2Codes(paramDecCodes, varListCode);
    }else{
        assert(0);
    }
}
...
InterCodes translate_VarDec(GramTree* root){
    ...
    int isAddr = true;
    //printf("VarDec:%s,%d\n",root->child[0]->val.str,root->child[0]->symIndex);
...
InterCodes translate_CompSt(GramTree* root){
    ...
    InterCodes a = translate_DefList(defList);
    InterCodes b = translate_StmtList(stmtList);
    return link2Codes(a,b);
...
InterCodes translate_DefList(GramTree* root){
    ...
    InterCodes a = translate_Def(root->child[0]);
    InterCodes b = translate_DefList(root->child[1]);
    return link2Codes(a,b);
    // return link2Codes(
...
InterCodes translate_Dec(GramTree* root){
    ...
    // p = p->child[0];    // ID所在节点 
    //printSymbolList();
    //printf("p->symIndex = %s,%d\n", p->child[0]->val.str,p->child[0]->symIndex);
    SymbolElem mysymbol = findFromList(p->child[0]->symIndex);
    //printSymbolElem(mysymbol);
    InterCodes c1 = NULL;
    InterCodes c2 = NULL;
    if(mysymbol->u.var->kind != BASIC){
        int isAddr = true;
        //printf("!!!!!!!!:%d,%s,%d\n",mysymbol->u.var->kind,p->tag,p->child[0]->symIndex);
        iPrintProduction(p);
        Operand op = getVar(p->child[0]->symIndex, isAddr);
        int tempsize = getTypeSize(mysymbol->u.var);
        //printf("getTEMP size: %d\n",tempsize);
    ...
    if(root->nChild == 3){
        printError("186");
        int isAddr = false;
        Operand var = getVar(p->child[0]->symIndex, isAddr);
        Operand op;
...
int getTypeSize(Type type){
    ...
    if(type->kind == BASIC){
        ...
        printError("?");
        //printf("structure?\n");
        return getFieldListSize(type->u.structure->tail);
        printError("?");
    }
    return 0;
}
...
InterCodes translate_Exp(GramTree* root, Operand* place){
    ...
    if(n==1){
        ...
        *place = res;
        ...
        //printf("Process Args Finish!\n");
        if (place != NULL)
            *place = res;
        InterCodes code3 = getCallCodes(res, funcName);
        //printf("Process Args Finish!\n");
...
InterCodes translate_StmtList(GramTree* root){
    ...
    if(root->nChild == 0 || root->nChild == 1){
        ...
        InterCodes a = translate_Stmt(root->child[0]);
        InterCodes b = translate_StmtList(root->child[1]);
        return link2Codes(a,b);
        // return link2Codes(
