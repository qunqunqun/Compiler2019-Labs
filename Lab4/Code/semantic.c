...
int insert_symbol_List(SymbolElem p){
    ...
    //start 赋值
    //printf("insertList: symIndex=%d, sym->name=%s,kind:%d\n", global_Symbol_Index, p->name,p->u.var->kind);
    //printSymbolElem(p);
...
FieldList Handle_VarList(GramTree* root){
    ...
    while(root!=NULL){
        ...
        GramTree* curVarDec = root->child[0]->child[1];
        while (curVarDec->nChild == 4)
            curVarDec = curVarDec->child[0];
        curVarDec->child[0]->symIndex = symbol->symIndex;
...
int insert_Type_List(Type type){
    ...
    if(global_Type_Index > MAX_TYPENUM) {
        assert(0);
    }
    global_Type_Index++;
    //printf("global_Type_Index:%d\n",global_Type_Index);
